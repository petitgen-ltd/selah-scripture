...
| `test_live_wiring.py` | Live integration wiring with an injected fake `requests` (no network): asserts the correct YouVersion (`/verses/{ref}`) and Gloo (`/chat/completions`) endpoints, `Bearer` auth headers, translation/language params, and prompt shape — proves live mode is wired correctly without real keys. |
